...
from saas.services import CustomerSuccessService, SubscriptionService
from shared.database import get_db
from shared.dependencies import get_current_user
from shared.response import success_response
from shared.tenant import is_super_admin, require_super_admin
...
async def platform_overview(
    ...
    from ecosystem.models import APIKey
    ...
    active_orgs = db.execute(
        select(sa_func.count(Organization.id)).where(Organization.is_deleted == 0, Organization.is_active == 1)
    ).scalar() or 0
    total_users = db.execute(
        select(sa_func.count(User.id)).where(User.is_deleted == 0)
    ).scalar() or 0
    active_users = db.execute(
        select(sa_func.count(User.id)).where(User.is_deleted == 0, User.is_active == 1)
    ).scalar() or 0
    total_subscriptions = db.execute(
        select(sa_func.count(Subscription.id)).where(Subscription.status.in_(["active", "trial"]))
    ).scalar() or 0
...
async def get_tenant_detail(
    ...
    users = db.execute(
        select(User).where(User.organization_id == org_id, User.is_deleted == 0)
    ).scalars().all()
    ...
    return success_response({
        ...
        "users": [
            {"id": u.id, "email": u.email, "full_name": u.full_name, "is_active": bool(u.is_active)}
            for u in users
        ],
