    ...
    def analyze(cls, df: pd.DataFrame, col_mapping: dict | None = None) -> AnalyticsResult:
        ...
        if grade_col and grade_col in df.columns:
            ...
            if grade_bd:
                ...
                fail_grades = [
                    g
                    for g in grade_bd.values
                    if any(fg in str(g).upper() for fg in ("F", "FAIL", "0", "1"))
                ]
                if fail_grades:
                    fail_count = sum(
                        grade_bd.values.get(fg, 0)
                        for fg in grade_bd.values
                        ...
                        insights.append(
                            Insight(
                                ...
                                alert=(
                                    "critical"
                                    if fail_rate > 20
                                    else "warning" if fail_rate > 10 else "ok"
                                ),
                            )
                        )
            ...
            if pd.api.types.is_numeric_dtype(df[attendance_col]):
                ...
                insights.append(
                    Insight(
                        ...
                        formatted=(
                            cls._fmt_pct(avg_attendance)
                            if avg_attendance <= 100
                            else cls._fmt_number(avg_attendance)
                        ),
                        category="academic",
