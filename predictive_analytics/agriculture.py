...
class AgriculturePredictiveAnalytics(PredictiveAnalyticsBase):
    ...
    def analyze(self, df: pd.DataFrame, col_mapping: dict) -> PredictiveIntelligenceResult:
        ...
        production_col = self._find_numeric_col(
            df, col_mapping, ["production", "harvest", "output", "yield"]
        )
        ...
        yield_col = self._find_numeric_col(
            df, col_mapping, ["yield", "yield_per_hectare", "productivity"]
        )
        if yield_col:
            feature_cols = []
            for entity in [
