...
this engine. No subsystem may independently decide chart types.
...
Usage:
  ...
  result = engine.generate(df, dataset_name="sales_data", industry="retail")
  dashboard = result["dashboard"]     # DashboardSpecification
  presentation = result["presentation"]  # PresentationSpecification
  charts = result["charts"]           # list[ChartSpecification]
  understanding = result["understanding"]  # DatasetUnderstanding
...
class VisualizationIntelligenceEngine:
    """The single canonical visualization engine.

    This facade wraps the existing auto pipeline and adds:
      - Chart validation with fallback
      - Schema versioning
      - A single entry point for all visualization needs
      - Report chart specification generation from canonical specs

    Every visualization-producing subsystem must use this engine.
    """
    ...
    def generate(
        ...
        """Run the full automatic visualization pipeline.
        ...
        Returns:
            Dict containing:
              ...
              - charts: list[ChartSpecification] (validated)
        ...
        # 3. Validate charts and apply fallback
        charts = self.validator.validate_and_fallback(charts, df)
        ...
        # 7. Generate recommendations from insights
        ...
        # 9. Generate presentation layout (uses SAME charts)
    ...
    def generate_chart_specs(
        ...
        """Generate only validated chart specifications.

        This is the entry point for subsystems that need chart specs
        without a full dashboard (e.g., report engine).
        """
        dataset_hash = self._compute_hash(df)
        understanding = self.analysis_engine.analyze(
            df,
        ...
        charts = self.chart_engine.select_charts(df, understanding, max_charts=max_charts)
        charts = self.validator.validate_and_fallback(charts, df)
        return charts

    def explain_chart(self, chart: ChartSpecification) -> str:
        """Return the 'Why this chart?' explanation in plain language."""
    ...
    def validate_chart(
        ...
        """Validate a single chart against a DataFrame.

        Returns:
            Tuple of (is_valid, reason).
        """
        result = self.validator.validate(chart, df)
