...
class ChartRecommendationEngine:
    ...
    def recommend_charts(
        ...
        for date_col in datetime_cols[:2]:
            for metric_col in metric_cols[:3]:
                charts.append(self._make_chart(
    ...
    def _find_geo_columns(col_mapping: dict, categorical_cols: list[str]) -> list[str]:
        ...
        return [col for col, ent in col_mapping.items() if ent in geo_entities and col in categorical_cols]

    @staticmethod
    def _find_metric_columns(df: pd.DataFrame, col_mapping: dict, numeric_cols: list[str]) -> list[str]:
        """Find columns that serve as metrics (measures)."""
        metric_entities = {
            "revenue", "expense", "offering", "tithe", "donation", "billing",
            "production", "downtime", "crop", "livestock", "weather",
            "transaction", "loan", "card", "claim", "policy",
            "reservation", "room", "service", "call", "data_usage",
        }
    ...
    def _find_dimension_columns(col_mapping: dict, categorical_cols: list[str]) -> list[str]:
        ...
        dimension_entities = {
            "region", "department", "department_edu", "ward", "course",
            "diagnosis", "event", "product", "program", "project",
            "category", "branch_church", "ministry", "plan",
        }
    ...
    def _find_entity_columns(col_mapping: dict) -> list[str]:
        ...
        entity_entities = {
            "patient", "student", "member", "customer", "donor",
            "beneficiary", "citizen", "account", "subscriber", "guest",
            "doctor", "teacher", "agent", "supplier",
        }
