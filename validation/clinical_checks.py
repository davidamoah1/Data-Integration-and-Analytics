...
class ClinicalValidationEngine:
    ...
    def _check_bp_ranges(df: pd.DataFrame) -> list[BusinessRuleFinding]:
        findings = []
        systolic_col = ClinicalValidationEngine._find_col(df, ["systolic", "bp_systolic", "systolic_bp"])
        diastolic_col = ClinicalValidationEngine._find_col(df, ["diastolic", "bp_diastolic", "diastolic_bp"])
        ...
        if systolic_col and diastolic_col and systolic_col in df.columns and diastolic_col in df.columns:
            if pd.api.types.is_numeric_dtype(df[systolic_col]) and pd.api.types.is_numeric_dtype(df[diastolic_col]):
    ...
    def _check_bmi(df: pd.DataFrame) -> list[BusinessRuleFinding]:
        ...
        if weight_col and height_col and weight_col in df.columns and height_col in df.columns:
            if pd.api.types.is_numeric_dtype(df[weight_col]) and pd.api.types.is_numeric_dtype(df[height_col]):
                w = df[weight_col]
                h = df[height_col]
    ...
    def _check_impossible_lab_values(df: pd.DataFrame) -> list[BusinessRuleFinding]:
        ...
        glucose_col = ClinicalValidationEngine._find_col(df, ["glucose", "blood_sugar", "blood_glucose"])
        if glucose_col and glucose_col in df.columns and pd.api.types.is_numeric_dtype(df[glucose_col]):
