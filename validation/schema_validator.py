...
HOSPITAL_SCHEMAS: dict[str, dict] = {
    "patient_registry": {
        ...
        "optional_columns": [
            "patient_name", "date_of_birth", "gender", "age", "phone",
            "email", "address", "blood_type", "weight", "height",
        ],
    },
    "admission_records": {
        "required_columns": ["patient_id", "admission_date"],
        "optional_columns": [
            "admission_id", "discharge_date", "ward", "doctor",
            "diagnosis", "department", "insurance_type", "amount",
        ],
    },
    "laboratory_results": {
        "required_columns": ["patient_id", "test_name"],
        "optional_columns": [
            "lab_id", "test_result", "unit", "reference_range",
            "specimen_date", "ordered_by", "status",
        ],
    },
    "medication_records": {
        "required_columns": ["patient_id", "medication_name"],
        "optional_columns": [
            "prescription_id", "dosage", "frequency", "route",
...
class SchemaValidator:
    ...
    def validate(
        ...
        numeric_like_cols = [
            c for c in df.columns
            if any(kw in c.lower() for kw in ["amount", "cost", "price", "weight", "height", "age", "rate", "count", "value"])
        ]
