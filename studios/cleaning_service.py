...
class DataCleaningService:
    ...
    def analyze_dataset(self, df: pd.DataFrame) -> dict:
        ...
        for col in df.columns:
                        ...
                        issues.append(
                            {
                                ...
                                "count": sum(
                                    ...
                                    if str(v).lower().strip() in COUNTRY_NORMALIZATION
                                ),
                                "examples": list(unique_vals[:10]),
            ...
            # 4. Invalid dates
            if col_data.dtype == "object":
                date_like = col_data.apply(
                    lambda x: self._is_date(str(x)) if pd.notna(x) else False
                )
            ...
            if col_data.dtype == "object":
                numeric_like = col_data.apply(
                    lambda x: self._is_numeric(str(x)) if pd.notna(x) else False
                )
        ...
        for t in approved_transformations:
            ...
            if action == "fill_missing":
                if t["method"] == "mean":
                    ...
                    changes_log.append(
                        f"Filled {before} missing values in '{col}' with mean {t['value']}"
                    )
                elif t["method"] == "mode":
                    before = df[col].isna().sum()
                    df[col] = df[col].fillna(t["value"])
                    changes_log.append(
                        f"Filled {before} missing values in '{col}' with mode '{t['value']}'"
                    )
