...
async def verify_api_key(request: Request, db: DbSession = Depends(get_db)) -> dict:
    ...
    if not raw_key:
        ...
        raise HTTPException(
            status_code=401, detail="API key required. Provide via X-API-Key header."
        )
...
async def public_upload_dataset(
    ...
    etl = ETLService(db)
    result = etl.upload_file(
        file.file, file.filename, organization_id=api_key_info["organization_id"]
    )
